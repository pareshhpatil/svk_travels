...
class LogsheetModel extends Model {
    ...
    public function saveLogsheetbill($vehicle_id, $company_id, $driver_id, $date, $bill_date, $start_km, $end_km, $start_time, $close_time, $daynight, $remark, $toll_amount, $type, $pick_drop, $from, $to, $user_id) {
        try {
            $sql = "INSERT INTO `logsheet_bill`(`vehicle_id`,`company_id`,`driver_id`,`date`,`bill_date`,`start_km`,`end_km`,`total_km`,`start_time`,`close_time`,`day_night`,`remark`,`toll`,`pick_drop`,`from`,`to`,`type`,`created_by`,`created_date`,`last_update_by`)VALUES(:vehicle_id,:company_id,:driver_id,:date,:bill_date,:start_km,:end_km,:total_km,:start_time,:close_time,:day_night,:remark,:toll,:pick_drop,:from,:to,:type,:user_id,CURRENT_TIMESTAMP(),:user_id);";
            $params = array(':vehicle_id' => $vehicle_id, ':company_id' => $company_id, ':driver_id' => $driver_id, ':date' => $date, ':bill_date' => $bill_date, ':start_km' => $start_km, ':end_km' => $end_km, ':total_km' => $end_km - $start_km, ':start_time' => $start_time, ':close_time' => $close_time, ':day_night' => $daynight, ':remark' => $remark, ':toll' => $toll_amount, ':pick_drop' => $pick_drop, ':from' => $from, ':to' => $to, ':type' => $type, ':user_id' => $user_id);
...
VALUES(:inv_id,:particular,:date,:qty,:rate,:amount,:user_id,CURRENT_TIMESTAMP(),:user_id);";
    ...
    public function saveLogsheetInvoice($vehicle_id, $company_id, $date, $bill_date, $fix_qty, $fix_rate, $fix_amt, $extra_day_qty, $extra_day_rate, $extra_day_amt, $extra_km_qty, $extra_km_rate, $extra_km_amt, $extra_hr_qty, $extra_hr_rate, $extra_hr_amt, $br_down_qty, $br_down_rate, $br_down_amt, $toll,$type, $user_id) {
        try {
            $invoice_number = $this->getInvoiceNumber();
            $sql = "INSERT INTO `logsheet_invoice`(`invoice_number`,`vehicle_id`,`company_id`,`date`,`bill_date`,`fix_qty`,`fix_rate`,`fix_amt`,`extra_day_qty`,`extra_day_rate`,`extra_day_amt`,`extra_km_qty`,`extra_km_rate`,`extra_km_amt`,`extra_hr_qty`,`extra_hr_rate`,`extra_hr_amt`,`br_down_qty`,`br_down_rate`,`br_down_amt`,`toll`,`type`,`created_by`,`created_date`,`last_update_by`,`last_update_date`)
                VALUES(:invoice_number,:vehicle_id,:company_id,:date,:bill_date,:fix_qty,:fix_rate,:fix_amt,:extra_day_qty,:extra_day_rate,:extra_day_amt,:extra_km_qty,:extra_km_rate,:extra_km_amt,:extra_hr_qty,:extra_hr_rate,:extra_hr_amt,:br_down_qty,:br_down_rate,:br_down_amt,:toll,:type,:user_id,CURRENT_TIMESTAMP(),:user_id,CURRENT_TIMESTAMP());";
            $params = array(':invoice_number' => $invoice_number, ':vehicle_id' => $vehicle_id, ':company_id' => $company_id, ':date' => $date, ':bill_date' => $bill_date, ':fix_qty' => $fix_qty, ':fix_rate' => $fix_rate, ':fix_amt' => $fix_amt, ':extra_day_qty' => $extra_day_qty, ':extra_day_rate' => $extra_day_rate, ':extra_day_amt' => $extra_day_amt, ':extra_km_qty' => $extra_km_qty, ':extra_km_rate' => $extra_km_rate, ':extra_km_amt' => $extra_km_amt, ':extra_hr_qty' => $extra_hr_qty, ':extra_hr_rate' => $extra_hr_rate, ':extra_hr_amt' => $extra_hr_amt, ':br_down_qty' => $br_down_qty, ':br_down_rate' => $br_down_rate, ':br_down_amt' => $br_down_amt, ':toll' => $toll, ':type' => $type, ':user_id' => $user_id);
            $this->db->exec($sql, $params);
        } catch (Exception $e) {
            SwipezLogger::error(__CLASS__, '[E295+54]Error while saveLogsheetbill Error: ' . $e->getMessage());
            $this->setGenericError();
    ...
    public function updateLogsheetInvoice($invoice_id, $vehicle_id, $company_id, $date, $bill_date, $fix_qty, $fix_rate, $fix_amt, $extra_day_qty, $extra_day_rate, $extra_day_amt, $extra_km_qty, $extra_km_rate, $extra_km_amt, $extra_hr_qty, $extra_hr_rate, $extra_hr_amt, $br_down_qty, $br_down_rate, $br_down_amt, $toll, $user_id) {
        try {
